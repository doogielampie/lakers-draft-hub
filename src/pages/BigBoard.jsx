    ...
    <div style={{ background: SURFACE, minHeight: '100vh' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding }}>
        ...
        <p style={{ color: MUTED, fontSize: 14, marginBottom: 8 }}>
          {isMobile
            ? 'Tap any card to open profile. Gold = Lakers targets.'
        ...
        {isMobile && (
          <div style={{ marginBottom: 12 }}>
            <div style={{
              ...
              {SORT_PILLS.map(({ col, label }) => (
                <button
                  key={col}
                  onClick={() => handleSort(col)}
                  style={{
                    background: sortCol === col ? GOLD : '#1a1a26',
                    color: sortCol === col ? '#000' : MUTED,
                    border: `1px solid ${sortCol === col ? GOLD : '#2a2a3a'}`,
            ...
            {/* Mobile pos filter row */}
            <div style={{ display: 'flex', gap: 5, overflowX: 'auto', scrollbarWidth: 'none', paddingBottom: 4 }}>
              {POSITIONS.map(p => (
                <button key={p} onClick={() => setPosFilter(p)} style={{
                  background: posFilter === p ? GOLD : '#1a1a26', color: posFilter === p ? '#000' : MUTED,
                  border: `1px solid ${posFilter === p ? GOLD : '#2a2a3a'}`,
                  borderRadius: 4, padding: '3px 8px', fontSize: 10, cursor: 'pointer', flexShrink: 0,
                  fontFamily: "'DM Mono', monospace",
                }}>{p}</button>
              ))}
            </div>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    borderLeft: `3px solid ${isTarget ? GOLD : inWindow ? `${GOLD}55` : BORDER}`,
                ...
                >
                  ...
                  <div style={{
                    ...
                    color: isTarget ? GOLD : MUTED, minWidth: 28, textAlign: 'center', flexShrink: 0,
                  }}>
                    {p.rd || '—'}
                  </div>

                  {/* Logo */}
                  <Logo logoKey={logoKey} size={28} fallback={p.sch} />
                  ...
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    {p.bpm != null && (
                      <div style={{
                        ...
                        color: p.bpm >= 10 ? GOLD : MUTED,
                        background: p.bpm >= 10 ? `${GOLD}18` : 'transparent',
                        padding: p.bpm >= 10 ? '2px 6px' : '0',
                        borderRadius: 4,
                      }}>
                        {fmt(p.bpm)}
                      </div>
                    )}
                    <div style={{ fontSize: 9, color: `${MUTED}88`, marginTop: 2, fontFamily: "'DM Mono', monospace" }}>
                      {p.bpm != null ? 'BPM' : ''}
                    </div>
                  </div>
